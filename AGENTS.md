...
1. 开发中新增了约定、踩了坑、总结了教训 → **在本次改动内**同步写入本文件（与代码改动同一次提交，禁止"先欠着，以后补"）
2. 发现本文件与代码实际行为不一致 → 立即修正规范（文档和实现不一致时，以实际验证过的行为为准修正文档）
3. 修改了 README 的功能/设置/验收清单 → 检查规范是否受影响，受影响则一并更新
4. 新增规范本身也要遵守上面的流程——本规则由 2026-08 误改 275 文件事故总结而来
